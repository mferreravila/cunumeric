...
template<typename OUTPUT_TYPE, typename INPUT_TYPE>
struct cufft_axes_plan_r2c<3, OUTPUT_TYPE, INPUT_TYPE>{
  __host__ static inline void execute(cufftHandle plan,
                                      //AccessorWO<OUTPUT_TYPE, 3> out,
...
// N-dimensional FFTs using this approach.
template <int DIM, typename OUTPUT_TYPE, typename INPUT_TYPE>
__host__ static inline void cufft_1d_operation(cudaStream_t stream,
                                               //AccessorWO<OUTPUT_TYPE, DIM> out,
                                               //AccessorRO<INPUT_TYPE, DIM> in,
                                               DeferredBuffer<OUTPUT_TYPE, DIM>& out,
                                               DeferredBuffer<INPUT_TYPE, DIM>& in,
                                               ...
                                               fftDirection direction) 
{
  ...
  int size_1d = n[axis];
  // TODO: batches only correct for DIM <= 3. Fix for N-DIM case
  int batches = (DIM == 3 && axis == 1) ? n[2] : num_elements_in / fft_size_in[axis];
  printf("BATCHES=%u\n", batches);
  int istride = 1;
  int ostride = 1;
  ...
  cufft_axes_plan_r2c<DIM, OUTPUT_TYPE, INPUT_TYPE>::execute(plan, out, in, out_rect, in_rect, axis, direction);
...
// std::raise(SIGINT);
    ...
    auto ax = axes.begin();
    cufft_1d_operation<DIM, OUTPUT_TYPE, INPUT_TYPE>(stream, output_buffer, input_buffer, out_rect, in_rect, *ax, type, direction);
    ++ax;

    for(; ax != axes.end(); ++ax) {
      cufft_1d_operation<DIM, OUTPUT_TYPE, OUTPUT_TYPE>(stream, output_buffer, output_buffer, out_rect, in_rect, *ax, fftType::FFT_Z2Z, direction);
    }
